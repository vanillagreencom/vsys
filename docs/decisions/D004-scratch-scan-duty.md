...
**Verification**: `bun run bench:scratch`, which is in the check contract. `DEVELOPMENT.md` says what it proves and what it refuses. The invariants in [storage architecture](../architecture/storage.md) name the tests, and are the one place those citations are kept.
