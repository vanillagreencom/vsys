...
- One pane on that server is vsys's own, the one it draws in. `lanes()` marks the lane holding it in the two forms a lane carries a pane: the `%N` handle from `TMUX_PANE`, matched only where the lane names the server vsys is attached to, because a handle means nothing across servers; and the `session:window.pane` address from `VSYS_PANE`, matched against the address vsys's own handle resolves to. A target naming a window rather than an index, such as `vsys:build.1`, is a third spelling of the same pane and matches neither.
- The handle vsys's own pane carries is read from vsys's own environment, not from the server's answer, so the marking survives a `list-panes` that failed, a server that is not running and a tmux that is not installed. Only the address form needs the read, because only the map says which pane an address names.
...
16. The lane holding the pane vsys draws in is marked as its own in both forms a lane can carry it, only where the lane's server is known and is vsys's own, and still when the pane read failed. `src/model/lanes.test.ts` checks a handle, an address, a vsys outside tmux, the same handle on another server, the same handle on no server, and an empty map; `src/collect/tmux.test.ts` checks what one read carries; `src/collect/collector.test.ts` checks a sample whose read answered and one whose read failed.
