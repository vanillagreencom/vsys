...
- `launcherTrail()` in `src/model/launcher.ts` states one process. `launcherCopy()` states a card: it groups the trails by conclusion and scope, because a lane holds many processes and each of them repeats the same markers, the same scope and the same ancestors.
...
5. One conclusion in one scope is one sentence, naming its markers once, its scope once with the count of processes in it, and the chain once for an example process it names by PID. `src/model/launcher.test.ts` groups six processes in one scope and ten over two.
