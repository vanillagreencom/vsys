...
5. One sentence covers every process agreeing on all four facts it states: the conclusion, the scope, the markers set on them and the PATH entries ahead of the login shell's own. It names the markers once, the scope once with the count of processes in it, and the ancestors once for an example process it names by PID. `src/model/launcher.test.ts` groups six processes in one scope, ten over two, and two pairs differing only by marker set and only by PATH prefix.
