...
- The pane handle and window title are read from the pane environment, and a pane that exports neither leaves both parts out of the lane name. A `%N` handle is resolved to an address and a window name by the tmux server, in one `list-panes` for the whole machine rather than one call per lane; a pane that exported an address rather than a handle keeps it whether or not a server answers, so an empty address never says tmux is absent.
...
- A pane handle is resolved only against the server it belongs to. `%9` is unique per tmux server, so the server vsys reads holds a `%9` of its own and resolving one against the other names a stranger's pane. The lane carries the `TMUX` its own shell exported, read from an environment the collector already parses, and a handle whose server is known to differ resolves to nothing and says so. An unknown server on either side is not a boundary vsys can see, so it refuses at neither: the only route to a handle carrying no server is a reader setting one by hand.
- What tmux is asked to do takes a target, not a handle: `-t` accepts `session:window.pane` as readily as `%9`, so a reader who configured `VSYS_PANE` gets a terminal that works rather than a refusal about the value they were told to set. `isPaneId` decides one thing, whether a handle can be looked up in a map keyed by `%N`. The copied `switch-client` line is quoted, because that one goes to a shell; the arguments vsys spawns are separate and take none.
...
- `distinctNames()` in `src/model/naming.ts` is the only rule for names that repeat, and lanes no longer use it: a lane name is never added to, and two lanes resolving to one name are told apart by the process id column beside them. Its one caller is Resources, which calls it over every group so that hiding the idle rows cannot rename a row the reader is looking at.
...
- No two resource groups in one sample render the same name. A colliding set takes the first candidate that separates every member of it: its parent, then its first process id, then its path. `src/ui/resources.test.tsx` checks each step and that the result is distinct. Two lanes can share a name, and the process id column tells them apart.
