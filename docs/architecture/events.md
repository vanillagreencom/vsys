...
- No event subject reaches a screen as a systemd unit name, and the unit it decoded from stays on the event. A build before this held wrote the raw unit as the subject, and those points stay in the ring for a retention window after an upgrade, so `normalizePoint()` in `src/store/migrate.ts` decodes them on the load path. It decodes only where the record proves what it holds: an alert or verdict, with no unit field, whose subject is the last segment of its own cgroup-path identity, which is how `collectGroups()` names a group. A lane name or a mount path that ends in a unit suffix is left as written. `src/store/events.test.ts` checks a scope subject against a lane subject and `src/store/history.test.ts` checks a stored point holding all four against each other.
