...
- Replay reads only the lines it does not already hold. The cursor keeps the snapshot it last rebuilt and the reader that inflated the lines it walked, and the newest lines are the open ones, so advancing to the newest sample decompresses nothing.
