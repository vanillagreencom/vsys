...
- One sealed segment at a time is inflated while a checkpoint is read, and a reader that walks a checkpoint inflates each of its segments once. A reader made before a seal cannot place a line after it and is built again.
...
- Replay reads only the lines it does not already hold. The cursor keeps the snapshot it last rebuilt and the reader that inflated the lines it walked. Advancing to the newest sample decompresses nothing, because the newest lines are the open ones and the open run is never compressed. A walk across a sealed checkpoint does decompress: the reader inflates a segment each time the walk leaves the one it holds, which is once per segment over a whole checkpoint rather than once per sample.
...
6. A sealed line is read at the line it is, and a walk through a checkpoint inflates each of its segments once rather than once per sample. `src/store/archive.test.ts` replays every sample of a checkpoint that sealed three times, compares a warmed lane chart against a cold one, and counts the inflations a full replay takes.
7. A checkpoint that rolled over is charged what it compressed to, so retention is never shortened inside a budget it did not reach. `src/store/archive.test.ts` fills four checkpoints and a copy of them against a three-megabyte budget.
