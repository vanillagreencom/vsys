...
7. A checkpoint that rolled over is charged what it compressed to, so retention is never shortened inside a budget it did not reach. `src/store/archive.test.ts` fills four checkpoints and a copy of them against a 3 MiB budget.
