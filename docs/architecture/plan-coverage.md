...
| [Settings](settings.md) | Probed capabilities, unreadable sources on request, then validated settings by group in one or two columns, with a name filter and in-place editing | Configuration, `src/ui/settings.test.ts`, `src/ui/settings-screen.test.ts`, `src/ui/App.test.tsx` and runtime replacement tests |
