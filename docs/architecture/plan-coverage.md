...
| [Agents](lanes.md) | Searchable rows naming account, agent, the resolved address and workspace with a CPU bar and a badge, the raw handle and the window name being the detail's to show; optional full table with sorting, column visibility and order | Collector fixtures, `src/model/naming.test.ts`, `src/ui/format.test.ts`, `src/ui/agents.test.tsx`, `src/ui/screen.test.tsx`, `src/ui/theme.test.tsx` |
