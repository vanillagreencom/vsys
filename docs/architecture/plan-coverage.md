...
| --- | --- | --- |
| Overview | One verdict line; four meters that each name their biggest consumer; attention cards grouped by cause with a next step and a copyable command; unreadable sources in a footer | `src/model/verdict.test.ts`, `src/model/roles.test.ts`, `src/model/launcher.test.ts`, `src/ui/overview.test.ts`, `src/ui/App.test.tsx` |
