...
| [Agents](lanes.md) | Searchable rows naming account, agent, pane and workspace with a CPU bar and a badge; optional full table with sorting, column visibility and order | Collector fixtures, `src/model/naming.test.ts`, `src/ui/format.test.ts`, `src/ui/agents.test.tsx`, `src/ui/screen.test.tsx`, `src/ui/theme.test.tsx` |
