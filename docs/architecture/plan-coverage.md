...
| Resources | Meter facts, swap, the cgroup tree with CPU and memory bars, idle leaves hidden until asked, limits and pressure for the selected group, and the raw unit name in its detail | `src/ui/resources.test.ts`, `src/model/naming.test.ts`, rendered navigation test |
