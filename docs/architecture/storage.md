...
- `scratchDutyPercent` is the share of one processor core a background traversal may hold. The traversal works for a slice, then rests for the remainder of that share before reading the next entry. A caller that waits for the reading gets the whole thread, because a script has no screen to protect. The reason the bound is a duty cycle rather than a stored index is [D004](../decisions/D004-scratch-scan-duty.md).
...
- Replacing the settings or closing the collector cancels the running traversal, and its reading is discarded rather than published under the settings that replaced it. The traversal reaches its thread's queue at every rest, which is how it hears the cancellation.
...
11. A traversal rests between entries, so it holds the share of a thread its duty grants and hears a cancellation while it runs. `src/collect/scratch-scan.test.ts` checks the rest a slice earns, that a paced traversal returns its thread, and that a stopped one reports no reading.
...
14. The scan thread answers with a complete reading, is kept between scans, and takes no work after it closes. `src/collect/scratch-worker.test.ts` checks all three and an abandoned scan.
