...
- The settings collection reads are declared in `src/collect/collector.ts`, and the runtime rebuilds the collector from that declaration alone. Display settings and notification rules stay out of it, because a rebuild discards the counters and alert state a sample compares against. `src/collect/collector.test.ts` checks the declaration against the collectors and the two model modules a sample derives from; `src/runtime.test.ts` checks that a collection setting rebuilds and a notification rule does not.
