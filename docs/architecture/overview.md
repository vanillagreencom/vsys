...
- Drive lifetime writes are parsed from `smartctl -A` reports written by a privileged timer, one file per `/sys/block` device name with a single optional extension. A drive without a readable report keeps unknown lifetime writes. `src/collect/devices.test.ts` checks NVMe data units, ATA logical blocks and a missing report.
- Every setting the collection modules read is declared in `src/collect/collector.ts`, and the runtime rebuilds the collector from that declaration. `src/collect/collector.test.ts` scans the modules for undeclared settings and `src/runtime.test.ts` checks the rebuild.
