...
- Bytes written since boot come from the io.stat read the Overview already uses, per slice and per device. A cgroup counts its whole subtree, so device totals come from the top of the tree and are never summed twice. `src/model/writes.test.ts` checks a nested subtree against the root and an unreadable counter.
- Drive lifetime writes are parsed from `smartctl -A` reports written by a privileged timer. A drive without a readable report keeps unknown lifetime writes. `src/collect/devices.test.ts` checks NVMe data units, ATA logical blocks and a missing report.
