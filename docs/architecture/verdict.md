...
- `integrities()` in `src/model/integrity.ts` is the one reading of whether a filesystem's data is damaged. The damaged-files cause, the unchecked cause and Storage all read it, so a card and a line cannot disagree about one filesystem.
...
12. A filesystem nothing has checked for damage raises a card, so silence about it never reads as health. `src/model/verdict.test.ts` checks the tie order of the full ladder, which holds both storage integrity causes.
