...
- Severity ranks the ladder, the cause order table breaks a tie, an unconfined agent leads it, and a housekeeping cause is a card but never the verdict. `src/model/verdict.test.ts` checks the ranking and `src/ui/attention.test.ts` checks a scratch overage on a healthy machine.
...
- One cause produces one attention card whatever the number of lanes, and every card ends with a next step distinct from its title and detail. `src/ui/attention.test.ts` checks nine stalling lanes and triggers every cause at once.
- Source read failures are counted once per source and stay out of attention. `src/ui/attention.test.ts` checks the footer.
