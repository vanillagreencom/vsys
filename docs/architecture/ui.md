...
- The pane vsys draws in is the one pane the detail never reads. Its capture would be this screen, nested one copy deeper on every sample, so the section says so instead and the `Go to terminal` row says the reader is already in it rather than offering a move tmux answers by doing nothing. Said only of a live sample: the mark was taken with the sample, and the vsys that recorded an older one may have been drawing somewhere else, so a pinned sample falls through to the line that says it is past.
