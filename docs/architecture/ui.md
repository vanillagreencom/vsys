...
- A confirmation is answered against the sample of the moment, never the one it opened on. A lane that ended, one whose scope name a later process took, one whose cgroup stopped resolving and one that moved to another scope each refuse and say which, and nothing runs. `src/model/actions.test.ts` covers the five answers; `src/ui/App.test.tsx` lands a sample under an open confirmation and checks both a lane that changed and one that did not.
