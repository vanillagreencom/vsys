...
- A list of forty lanes reads the history of the rows on screen and no others, reads none of them twice within a bucket, and is read again only when the chart's newest bucket rolls over, which is the soonest the drawn shape can change. Where the column is not drawn it is not read. A series is drawn on what it is an answer to, never on when it arrived, so a read still in flight from the previous bucket cannot put its window back on the screen. `src/ui/App.test.tsx` asserts the lane ids the store was asked for, not the rows drawn, so a change that widened the read fails even though the screen would look the same, and holds two reads across a bucket roll to answer out of order.
