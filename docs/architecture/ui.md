...
- `src/ui/attention.ts` turns the cause ladder and the meters into the verdict line, one card per cause and one tile per meter. Card copy is written against the width it is drawn at: `detailWidth` in `src/ui/chrome.tsx` is the one rule for that width, taking the terminal row through the screen padding, the panel and the rule and indent of the `Detail` block, and `wrapLines` and `capLines` in `src/ui/columns.ts` measure and cut against it. `capLines` cuts the text itself rather than rebuild it from wrapped rows, so a word too wide for the column comes back without a blank inside it.
- A card's detail is the conclusions it draws, the ancestors behind each of them, and one sentence naming the lanes the title above it cut. A card offers `fitDetail` its ways of writing the first part, best first, and the sentence it keeps; ground is given in that order, the ancestors first and then a conclusion at a time from the last, never the first, and every conclusion dropped is counted in what stays. At the floor a panel can be, where one whole conclusion and that count do not fit together, the last way of writing it states every conclusion as a count per kind, which is what the card can say there with nothing cut. The lane sentence holds two rows whatever the names in it: what it cannot fit it counts, and where not even one name fits beside that count the name gives way to it. It carries the process count beside the lane count wherever the two differ, so nothing states one number without the other.
...
28. A card's detail holds six rows at the width it is drawn at, at every width down to the floor `detailWidth` allows, whatever the number of affected lanes or processes. What it cannot hold it counts or marks: a dropped conclusion is counted in the groups of processes it spoke for, an unnamed lane in lanes, and a sentence cut mid-word ends in the mark. At the twenty-column floor it counts the conclusions by kind and cuts nothing. `src/ui/attention.test.ts` measures every cause the ladder can report, read from the ladder's own table, at a narrow and a wide panel; `src/ui/chrome.test.ts` pins the width against the constants the renderer draws with, and `src/ui/home.test.tsx` counts the rows drawn between a card and its `Next` line on a machine whose agents all escaped.
