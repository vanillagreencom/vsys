...
28. A card's detail holds six rows at the width it is drawn at, whatever the number of affected lanes or processes, and still names every lane its cut title named. `src/ui/attention.test.ts` measures every cause the ladder can report, read from the ladder's own table, at a narrow and a wide panel; `src/ui/chrome.test.ts` pins the width against the constants the renderer draws with, and `src/ui/home.test.tsx` counts the rows drawn between a card and its `Next` line on a machine whose agents all escaped.
