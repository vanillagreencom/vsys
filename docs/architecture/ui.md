...
- Actions are the one thing the UI does to the system, and only with `writeMode` on. `src/model/actions.ts` returns the command; the shell refuses it while `writeMode` is off and otherwise holds it behind a confirmation; `src/main.ts` performs it. A lane whose cgroup names no systemd scope gets no actions.
...
- No agent action reaches an effect while `writeMode` is off, and with it on the confirmation stands between the key and the call. `src/ui/App.test.tsx` plants an action hook that must not be called; `src/model/actions.test.ts` pins the exact command of each action.
