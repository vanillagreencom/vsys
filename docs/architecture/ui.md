...
- The pane vsys draws in is the one pane the detail never reads. Its capture would be this screen, nested one copy deeper on every sample, so the section names the pane instead and the `Go to terminal` row says the reader is already in it rather than offering a move tmux answers by doing nothing.
...
26. The lane holding the pane vsys draws in is captured zero times, switched to never, and copies no command, while every other pane is still read and still switched to. `src/ui/agent.test.tsx` counts the captures and the switches for both lanes.
