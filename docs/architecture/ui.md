...
- The detail never reads a lane the model marked as holding the pane vsys draws in; [lanes.md](lanes.md) states when that mark is made, and after a pane read that failed it is made in the handle form alone. Its capture would be this screen, nested one copy deeper on every sample, so the section says so instead and the `Go to terminal` row says the reader is already in it rather than offering a move tmux answers by doing nothing. Said only of a live sample: the mark was taken with the sample, and the vsys that recorded an older one may have been drawing somewhere else, so a pinned sample falls through to the line that says it is past.
...
26. A lane the model marked as holding the pane vsys draws in is captured zero times, switched to never, and copies no command, while every other pane is still read and still switched to; its section draws one explanation and no other, and a past sample of it is named as past. `src/ui/agent.test.tsx` counts the captures and the switches for both lanes and pins the section's whole message.
27. Process text cannot emit terminal controls. `src/ui/format.test.ts` checks the display sanitizer.
28. Terminal settings are restored on quit and on a failed shutdown alike. `src/main.test.ts` drives isolated terminals.
