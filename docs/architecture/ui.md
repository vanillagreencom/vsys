...
28. A card's detail holds six rows at the width it is drawn at, at every width down to the floor `detailWidth` allows, whatever the number of affected lanes or processes. What it cannot hold it counts or marks: a dropped conclusion is counted in cgroups, an unnamed lane in lanes, and a sentence cut mid-word ends in the mark. `src/ui/attention.test.ts` measures every cause the ladder can report, read from the ladder's own table, at a narrow and a wide panel; `src/ui/chrome.test.ts` pins the width against the constants the renderer draws with, and `src/ui/home.test.tsx` counts the rows drawn between a card and its `Next` line on a machine whose agents all escaped.
