...
- A card's detail is the conclusions it draws, the sentence reconciling a count in the title with a count under it, the ancestors behind each conclusion, and the list of lanes the title above it cut. Ground is given in that reverse order: the ancestors first, then the reconciliation, then a conclusion at a time from the last, never the first, and the lane list only down to the names the title itself showed. `fitDetail` picks the first way of writing the detail that holds the budget.
...
28. A card's detail holds six rows at the width it is drawn at, whatever the number of affected lanes or processes, and still names every lane its cut title named. Where those names alone need more rows than the budget, a panel of about thirty columns or fewer, the names are what it keeps. `src/ui/attention.test.ts` measures every cause the ladder can report, read from the ladder's own table, at a narrow and a wide panel; `src/ui/chrome.test.ts` pins the width against the constants the renderer draws with, and `src/ui/home.test.tsx` counts the rows drawn between a card and its `Next` line on a machine whose agents all escaped.
