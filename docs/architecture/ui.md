...
- `src/ui/attention.ts` turns the cause ladder and the meters into the verdict line, one card per cause and one tile per meter. Card copy is written against the width it is drawn at: `detailWidth` in `src/ui/chrome.tsx` gives the panel less the rule and the indent, and `wrapLines` and `capLines` in `src/ui/columns.ts` measure and cut against it. A detail is a lead that grows with the machine and a tail that must survive it, so the lead is cut first and the lane list under a cut title is never the part that goes.
...
28. A card's detail holds six rows at the width it is drawn at, whatever the number of affected lanes or processes, and still names every lane its cut title named. `src/ui/attention.test.ts` measures every cause at a narrow and a wide panel; `src/ui/home.test.tsx` opens a card on a machine whose agents all escaped and reads `Next` and the command off the frame.
