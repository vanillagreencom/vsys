...
- `scripts/lib/command-position.sh` is the text a shell command would run, cut into segments by `command_segments`; the catalog hooks `block-worktree-refresh` and `skill-load-check` source it from the install beside them, and their suites under `hooks/tests/` pin it.
